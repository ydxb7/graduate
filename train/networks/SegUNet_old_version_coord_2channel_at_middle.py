...
class coord_Block(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers):
        super(coord_Block, self).__init__()
        layers = [
...
class SegUNet_old_version_coord_at_middle(nn.Module):
    def __init__(self,num_classes):
        super(SegUNet_old_version_coord_at_middle, self).__init__()
        ...
        self.conv_coord = coord_Block(512+2, 512, 1)
        
        self.dec5 = SegUNetConvBlock(512, 512, 1)           # 16*16*512(after up) --> 32*32*512
    ...
    def forward(self, x, coord):
        ...
        #-------------------------------coord------------------------------------------------
        conv_coord = self.conv_coord(torch.cat([enc5_pool, coord], 1))
        #-------------------------------Decoder------------------------------------------------
        # Stage 5d
        # 16*16*512 --> 32*32*512
        dec5 = self.dec5(F.max_unpool2d(conv_coord, id5, kernel_size=2, stride=2)) 
